...
// Carefully replace the block
const startIndex = content.indexOf('export const parsePptx = async (arrayBuffer, sites = []) => {');
const endIndex = content.indexOf('// 🏷️ Ultra-Accurate Agency Logo & Watermark Identification');

if (startIndex !== -1 && endIndex !== -1) {
    const before = content.substring(0, startIndex);
    const after = content.substring(endIndex);
    fs.writeFileSync('src/core/pptxEngine.js', before + newParse + "\n\n  " + after);
    console.log('Successfully patched parsePptx');
} else {
